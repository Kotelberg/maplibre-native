...
// Fork-local directional-shadow light properties (cast-shadows, shadow-intensity); not in the public
// MapLibre style spec — see src/mbgl/style/light_impl.hpp / SHADOW_REWRITE_DESIGN.md §3.1.
TEST(StyleConversion, LightShadowProperties) {
    Error error;
    auto parseLight = [&](const std::string& src) { return convertJSON<Light>(src, error); };

    // Defaults: shadows on by default (preserves pre-P1 behavior), "subtle" 0.32 intensity.
    ASSERT_EQ(Light::getDefaultCastShadows(), true);
    ...
    {
        auto light = parseLight(R"({"cast-shadows":false,"shadow-intensity":0.6})");
        ASSERT_TRUE((bool)light);

        ASSERT_FALSE(light->getCastShadows().isUndefined());
        ASSERT_TRUE(light->getCastShadows().isConstant());
        ASSERT_EQ(light->getCastShadows().asConstant(), false);
