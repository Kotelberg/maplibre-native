...
/// groups). `MLN_SHADOW_CASCADE_COUNT` override; default 2 on Metal, 1 on GL/Vulkan (near = crisp
/// building/near shadows, far = pitched-horizon coverage; the multi-cascade path is only validated
/// on Metal so far). Clamped to [1, kMaxShadowCascades]; =1 reproduces the legacy single-map
