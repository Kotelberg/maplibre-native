...
/// groups). `MLN_SHADOW_CASCADE_COUNT` override; default 2 on every backend (near = crisp
/// building/near shadows, far = pitched-horizon coverage; a single far-fitted cascade is too
/// coarse for the building receiver at moderate zooms, so building-surface shadows would lag
/// ground shadows while zooming in). Clamped to [1, kMaxShadowCascades]; =1 reproduces the legacy
/// single-map path exactly. This is the ceiling; the per-frame ACTIVE count is `activeShadowCascadeCount`.
...
/// Renderer-owned directional-shadow pass (light-driven architecture).
///
...
/// caster drawables into a per-layer caster group (keyed by layer id); ALL receiver
...
/// several fill-extrusion layers (e.g. a base building layer plus hover/selection highlight layers).
///
