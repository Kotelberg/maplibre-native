...
float fe_unpackShadowDepth(float4 rgba) {
    const float d = dot(rgba, float4(1.0, 1.0/255.0, 1.0/65025.0, 1.0/16581375.0));
    ...
    // Vulkan/Mali), and 0.0 compares NEARER than every roof -> the entire far field is wrongly shadowed
    // (the grey-roof "D3" artifact). ShadowFrustum::fit pads the ortho near plane BELOW the tallest
    // caster (zPad), so no real occluder ever encodes depth ~0; treat a ~0 texel as FAR so an unwritten
    // sample never occludes. Behaviour-identical for every real caster (>= ~0.02) and the white seed.
...
fragment FragmentOutput fragmentMain(FragmentStage in [[stage_in]],
    ...
    for (int c = 0; c < in.cascade_count; ++c) {
        ...
        // shading is the directional light's job); sun-facing faces get ~0 bias (their light-space
        // depth gradient is flat, so there is no acne to suppress) so a neighbour's cast shadow still
        // lands on them.
