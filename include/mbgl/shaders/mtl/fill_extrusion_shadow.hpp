...
fragment FragmentOutput fragmentMain(FragmentStage in [[stage_in]],
    ...
    for (int c = 0; c < in.cascade_count; ++c) {
        ...
        // highest. Building receiver only (the ground receiver has its own bias); walls are
        // separately suppressed, so bias here can't leak onto walls.
        const float biasScale = 4.0;
