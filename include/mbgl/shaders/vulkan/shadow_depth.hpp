...
//     Skipping the y-flip also MIRRORS the apparent triangle winding in Vulkan's y-down
//     framebuffer space (every other Vulkan vertex stage ends with gl_Position.y *= -1.0, which
//     the fixed CullFaceWindingType -> vk::FrontFace mapping assumes). These shader sources
//     declare `skipsClipSpaceYFlip` so the backend flips vk::FrontFace for their pipelines,
//     keeping gfx::CullFaceMode meaning the same thing it means on every backend (without this
//     the roof caster's back-face culling culls the roofs instead and the shadow map loses its
//     nearest-to-light occluders).
...
struct ShaderSource<BuiltIn::ShadowDepthShader, gfx::Backend::Type::Vulkan> {
    ...
    // Light-space pass: no applySurfaceTransform()/y-flip, so apparent winding is mirrored — the
    // backend must flip vk::FrontFace for this shader's pipelines (see header comment).
    static constexpr bool skipsClipSpaceYFlip = true;
...
struct ShaderSource<BuiltIn::ShadowDepthInstancedShader, gfx::Backend::Type::Vulkan> {
    ...
    // Same light-space rasterization convention as the roof caster above: no y-flip, so the
    // backend flips vk::FrontFace for this shader's pipelines.
    static constexpr bool skipsClipSpaceYFlip = true;
