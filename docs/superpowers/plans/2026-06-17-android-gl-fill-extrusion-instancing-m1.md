...
- **Task 10 reuses the Metal instanced scaffolding via widened guards (not a separate block):** the roof keeps building via the existing non-instanced path (the gated bucket emits roof-only into sharedVertices/sharedTriangles); the walls reuse the SAME `instancedDepthBuilder`/`instancedColorBuilder`/`instanceVertexAttrs`/`finishInstance` by widening their guards to `MLN_USE_FILL_EXTRUSION_INSTANCING || MLN_GL_FE_INSTANCING`. Only the instance GEOMETRY binding differs: Metal binds the raw outline buffer + ed_discard (kept `#if MLN_USE`), while the GL path binds pos0/pos1/normal0/normal1/edgedistance from `bucket.sharedGLEdgeInstances` (`#if MLN_GL_FE_INSTANCING`, in the early read block). Data-driven base/height/color are read once into `instanceAttrs` for both. Read-before-flush ordering is preserved (reads in the early block, `finishInstance` flush late) — matching Metal exactly. The Metal shadow wall-caster prebuild + build are narrowed to `MLN_USE` (GL caster = Task 11). `GLEdgeInstanceVertex` is namespace-scope (`mbgl::`), not a bucket member.
- **Task 11 done in full (shadows ARE on for GL/Android, `MLN_DRAWABLE_SHADOWS=1`):** the hand-written `gl/shadow_depth_instanced.hpp` placeholder is replaced with a real edge-indexed caster (static quad + per-instance pos0/pos1 + data-driven base/height, `#ifndef HAS_UNIFORM_*` mirroring `gl/shadow_depth.hpp`). Registered like the FE instanced shaders (ShaderInfo + `instanceAttributes<ShadowDepthInstancedShader>` + renderer_backend, all gated; the registration the plan put in Task 8 lives here). Render layer: `shadowDepthInstancedGroup` member/fetch widened to the combined guard; a GL caster prebuild (read-before-flush) builds `glCasterInstanceAttrs` (base/height + pos0/pos1) + `glCasterStaticVertices`, and a per-cascade GL wall-caster build mirrors the Metal one. Needed because the gated bucket emits roof-only triangles, so the non-instanced ShadowDepthShader caster covers only the roof → ground shadow would detach without this. Compile-verified gate on/off; shadow correctness verified on-device (Task 12).
