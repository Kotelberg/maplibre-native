...
// Namespace hygiene: cgltf.h is a vendored single-header C library whose
// symbols are extern "C" (a C++ namespace can't shadow them), so any other
// translation unit that also links a copy of cgltf could collide with this
// one at link time. Rename every public cgltf function via macros before
// including the implementation to keep it uniquely ours.
...
BakedModel loadGlbMesh(const std::string& path) {
    ...
    // The engine's fixed bake light (azimuthal 210°, polar 45°, intensity
    // 0.35): a single hardcoded sun direction so every static-mesh model
    // shades consistently regardless of style. World axes: x east, y south,
    // z up; direction points toward the light. Ambient floor is 1 − intensity
    // = 0.65. Caveat: this is baked once in map space, not relit per frame —
    // under camera rotation it can drift from any style-driven lighting
    // (e.g. extrusion shading) elsewhere in the scene; per-frame relighting
    // from a style-declared light would need vertex normals on the GPU and
    // is future work.
