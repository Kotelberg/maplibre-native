...
/// A GLB baked once into a static, map-renderable mesh (parts merged per material): node
...
/// (the per-instance tweaker scales by `model-scale` meters, as the placeholder cube does).
/// Rendered unlit with baseColor textures — rock-solid under camera motion
/// and per-pixel depth-correct, at the cost of full PBR lighting (a baked
/// per-face lambert approximation stands in; see glb_mesh_loader.cpp).
struct BakedModel {
    struct Part {
