...
void FillExtrusionShadowTweaker::execute(LayerGroupBase& layerGroup, const PaintParameters& parameters) {
    ...
    const FillExtrusionShadowPropsUBO propsUBO = {
        ...
        // BUILDING receiver CONSTANT self-shadow bias removed (was 0.0015): on the single/far cascade
        // its fixed light-NDC offset buys a visible world-space gap between a wall and its cast shadow
        // on a neighbouring roof (peter-panning; the tight Metal near cascade masked it). Flat sun-
        // facing roofs have a near-zero light-space depth gradient, so there is no acne to suppress —
        // they are protected by the slope-scaled term below (~0 on those faces, large on grazing/away
        // faces). The GROUND receiver (flat, never casts) is already 0. MLN_SHADOW_BIAS env stays as
        // the escape hatch.
        ...
        // already darkens) so the building never self-shadows them into acne stripes, ~0 on
        // sun-facing faces so a neighbour's cast shadow still lands.
        .shadow_slope_bias = 0.05f};
