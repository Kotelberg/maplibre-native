...
uint32_t shadowCascadeCount() {
    static const uint32_t count = [] {
        ...
        // plus a far cascade for pitched-horizon coverage. Default 2 on Metal, 1 elsewhere (the
        // multi-cascade path is only validated on Metal so far). Clamped to [1, kMaxShadowCascades].
